...
//		}
		...
		private ProgressHUDMode? _mode;
		public ProgressHUDMode Mode {
			get {
				if (!_mode.HasValue) {
					_mode = ProgressHUDMode.Indeterminate;
		...
		public float Progress {
			...
			set {
				if (_progress != value)
					_progress = value;
				if (Mode == ProgressHUDMode.Determinate) {
					EnsureInvokedOnMainThread (() =>
					{
						UpdateProgress ();
						SetNeedsDisplay ();
		...
		{
			var indicator = Indicator as RoundProgressView;
			if (indicator != null) {
		...
		private void UpdateIndicators ()
		{
			...
			if (Mode == ProgressHUDMode.Determinate) {
				Indicator = new RoundProgressView ();
			} else {
		...
		public ProgressHUD (UIView view) : base(view.Bounds)
		{
			Initialize ();
		}
		...
		void Initialize ()
		{
			this.Mode = ProgressHUDMode.Indeterminate;
