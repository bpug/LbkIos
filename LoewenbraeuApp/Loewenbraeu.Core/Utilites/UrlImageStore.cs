...
namespace Loewenbraeu.Core
{
	...
	{
		void UrlImageUpdated (TKey id, bool local);
	}
	...
	public class UrlImageStore<TKey>
	{
		...
		public UrlImageStore (int capacity, string storeName, ProcessImageDelegate processImage)
		{
			...
			*/
			picDir = Path.Combine (baseDir, "Library/Caches/Pictures.UrlImageStore/");
			if (!string.IsNullOrEmpty (storeName)) {
				picDir = Path.Combine (baseDir, "Library/Caches/Pictures.UrlImageStore/" + storeName + "/");
			}
		...
		public UIImage RequestImage (TKey id, string url, IUrlImageUpdated<TKey> notify)
		{
			...
			if (File.Exists (picFile)) {
				...
				if (img != null) {					
					...
					//notify.UrlImageUpdated (id, true);
		...
		void DownloadImagesWorker (object state)
		{
			...
			while ((nextReq = GetNextRequest()) != null) {
				...
				try {
					img = UIImage.LoadFromData (NSData.FromUrl (NSUrl.FromString (nextReq.Url)));
				} catch {
				}
				...
				NSError err = null;
				try {
					string ext = Path.GetExtension (nextReq.Url);
					string picPath = picDir + nextReq.Id + ext;
					if (ext == ".png") {
						img.AsPNG ().Save (picPath, false, out err);
					} else {
						img.AsJPEG (0.8f).Save (picPath, false, out err);
					}
				} catch {
				}
				...
				nextReq.Notify.UrlImageUpdated (nextReq.Id, false);
			}
