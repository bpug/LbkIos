...
namespace Loewenbraeu.UI
{
	public class PagedViewController : UIViewController
	{
		...
		readonly UIPageControl _pageControl = new UIPageControl{
                  Pages = 0,
                  Frame = new RectangleF (0, 390, 320, 30)
            };
		...
		public PagedViewController (int startPage): this ()
		{
		...
		private void SetCurrentPage (int index, bool animated)
		{
			Page = index;
			_scrollView.SetContentOffset (new PointF ((index * 320), 0), animated);
			_pages [index].ViewDidAppear(true);
		...
		void HandleScrollViewDecelerationEnded (object sender, EventArgs e)
		{
			...
			Title = _pages [page].Title;
		}
           
		void HandlePageControlValueChanged (object sender, EventArgs e)
		{
			SetCurrentPage(_pageControl.CurrentPage, true);
		}
		...
		public void ReloadPages ()
		{
			...
			PagedViewDataSource.Reload ();
			if (_startPage > -1) {
				SetCurrentPage (_startPage, false);
				_startPage = -1;
			} else {
 ...
 */
		...
		{
			if (pageNumber < 0 || pageNumber >= this._pageControl.Pages)
				return;
			...
			if (UIDevice.CurrentDevice.Orientation == UIDeviceOrientation.LandscapeLeft ||
				...
				this._scrollView.ContentOffset = new PointF (pageNumber * 480, 0);
			} else {
				this._scrollView.ContentOffset = new PointF (pageNumber * 320, 0);
			}
